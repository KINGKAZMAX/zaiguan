...
| ♥ 心形收藏 | 红色 `#D61F2B` 爱心:卡片悬浮、最爱筛选、作品页按钮 |
| **Crop details(裁剪细节)** | Pointer Events 拖选 + Canvas 裁切;细节画廊 + 一句注 |
| 白底编辑排版 | 纯白底、大衬线标题(系统 Songti SC 栈)、黑色粗顶线统计条 |
| 黑色按钮/方形圆角 2px | `--ink #111` 按钮、细线面板、克制的灰阶层次 |

**裁剪细节实现**:拖选框用 `box-shadow: 0 0 0 9999px rgba(17,17,17,.38)` 实现选区外暗化;保存时按显示尺寸/原图尺寸比例换算后 Canvas `drawImage` 裁切,输出 ≤900px JPEG。
